...
void dotDetect(cv::Mat src,int p,Point3 dotMatrix[][80]){
...
void sortEveryRow(Point3 dotMatrix[][80], Point3 dstDotMatrix[][80],Size picSize){
	...
	for (int i = 0+topGap+bottomGap+sum_midd_row; i < 30 ; i++){
		...
		while (k<80 && dotMatrix[i][nn].Center.x!=0){
